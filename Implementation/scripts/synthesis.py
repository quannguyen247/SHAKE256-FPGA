...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run synthesis only for SHAKE256 Vivado project.")
    ...
    run_dir = LOGS_DIR / f"synthesis_{run_tag}"
    run_dir.mkdir(parents=True, exist_ok=True)
    log_file = run_dir / "synthesis.log"
    log_file.write_text(
        "SHAKE256 synthesis run\n"
        f"run_tag={run_tag}\n"
        f"run_dir={run_dir}\n",
        encoding="utf-8",
    )
    ...
    tcl_content = build_tcl(args.jobs)
    ...
    if rc == 0:
        print("[PASS] Synthesis completed successfully.")
        print(f"[INFO] Logs: {log_file}")
        return 0

    print(f"[FAIL] Synthesis failed (exit {rc}).")
    print(f"[INFO] Logs: {log_file}")
    return 1
